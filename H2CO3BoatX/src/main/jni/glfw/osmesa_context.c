...
static void makeContextCurrentOSMesa(_GLFWwindow* window)
{
    if (window)
    {
        ...
        if (strcmp(getenv("LIBGL_STRING"), "VirGLRenderer") != 0) {
            ANativeWindow_lock(window->boat.handle, &buf, NULL);
...
static void swapBuffersOSMesa(_GLFWwindow* window)
{
    ...
    if (strcmp(getenv("LIBGL_STRING"), "VirGLRenderer") == 0) {
        ...
        window->context.Finish();
        ANativeWindow_unlockAndPost(window->boat.handle);
        ANativeWindow_lock(window->boat.handle, &buf, NULL);
    }
}
...
GLFWbool _glfwInitOSMesa(void)
{
    ...
    if (strcmp(renderer, "VirGLRenderer") == 0) {
        ...
        sprintf(fileName, "%s/libvirgl_test_server.so", getenv("BOAT_NATIVEDIR"));
                                  ...
                                  const _GLFWfbconfig* fbconfig)
{
    ...
    if (strcmp(renderer, "VirGLRenderer") == 0) {
        ...
        ANativeWindow_setBuffersGeometry(window->boat.handle, 0, 0, vid);
