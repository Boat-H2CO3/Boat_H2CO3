...
@SuppressLint("NewApi")
internal fun FragmentActivity.navigationBar(
    config: BarConfig,
    block: (BarConfig.() -> Unit)? = null
) {
    if (!needApply) return
    if (block != null) {
        config.block()
