...
{
    // Use the pre-filled LUT (see createKeyTables() in h2co3_launcher_init.c)
    if (scancode < 0 || scancode > 255)
        return GLFW_KEY_UNKNOWN;

    return _glfw.h2co3_launcher.keycodes[scancode];
}
...
{
    _glfw.h2co3_launcher.disabledCursorWindow = window;
    _glfwPlatformGetCursorPos(window,
                              &_glfw.h2co3_launcher.restoreCursorPosX,
                              &_glfw.h2co3_launcher.restoreCursorPosY);
...
{
    _glfw.h2co3_launcher.disabledCursorWindow = NULL;
                                   ...
                                   const _GLFWwndconfig* wndconfig)
{
    ...
    window->h2co3_launcher.handle = h2co3GetNativeWindow();
    ANativeWindow_acquire(window->h2co3_launcher.handle);

    if (!window->h2co3_launcher.handle)
    {
    ...
    _glfw.h2co3_launcher.eventCurrent = window;

    if (!wndconfig->decorated)
        _glfwPlatformSetWindowDecorated(window, GLFW_FALSE);

    window->h2co3_launcher.maximized = GLFW_TRUE;
    _glfwPlatformSetWindowTitle(window, wndconfig->title);
    _glfwPlatformGetWindowPos(window, &window->h2co3_launcher.xpos, &window->h2co3_launcher.ypos);
    _glfwPlatformGetWindowSize(window, &window->h2co3_launcher.width, &window->h2co3_launcher.height);
...
{
    _GLFWwindow* window = _glfw.h2co3_launcher.eventCurrent;
    ...
    switch (event->type)
    {
        ...
        case MotionNotify:
        {
            ...
            if (x != window->h2co3_launcher.warpCursorPosX ||
                y != window->h2co3_launcher.warpCursorPosY)
            {
                // The cursor was moved by something other than GLFW

                if (window->cursorMode == GLFW_CURSOR_DISABLED)
                {
                    if (_glfw.h2co3_launcher.disabledCursorWindow != window)
                        return;
                    if (window->rawMouseMotion)
                        return;

                    const int dx = x - window->h2co3_launcher.lastCursorPosX;
                    const int dy = y - window->h2co3_launcher.lastCursorPosY;
            ...
            window->h2co3_launcher.lastCursorPosX = x;
            window->h2co3_launcher.lastCursorPosY = y;
            return;
        }
        ...
        case ConfigureNotify:
        {
            ...
            if (width != window->h2co3_launcher.width ||
                height != window->h2co3_launcher.height)
            {
                ...
                window->h2co3_launcher.width = width;
                window->h2co3_launcher.height = height;
            }
...
void _glfwPlatformDestroyWindow(_GLFWwindow* window)
{
    ...
    if (window->h2co3_launcher.handle)
    {
        ANativeWindow_release(window->h2co3_launcher.handle);
        window->h2co3_launcher.handle = NULL;
    }
}
...
{
    if (width)
        *width = ANativeWindow_getWidth(window->h2co3_launcher.handle);
    if (height)
        *height = ANativeWindow_getHeight(window->h2co3_launcher.handle);
}
...
{
    if (xscale)
        *xscale = _glfw.h2co3_launcher.contentScaleX;
    if (yscale)
        *yscale = _glfw.h2co3_launcher.contentScaleY;
}
...
void _glfwPlatformSetCursorPos(_GLFWwindow* window, double x, double y)
{
    ...
    window->h2co3_launcher.warpCursorPosX = (int) x;
    window->h2co3_launcher.warpCursorPosY = (int) y;
...
{
    if (mode == GLFW_CURSOR_DISABLED && _glfwPlatformWindowFocused(window))
    {
        disableCursor(window);
    }
    else if (mode != GLFW_CURSOR_DISABLED && _glfw.h2co3_launcher.disabledCursorWindow == window)
    {
        enableCursor(window);
    }
    // else
...
const char* _glfwPlatformGetScancodeName(int scancode)
{
    if (scancode < 0 || scancode > 0xff ||
        _glfw.h2co3_launcher.keycodes[scancode] == GLFW_KEY_UNKNOWN)
    {
//        _glfwInputError(GLFW_INVALID_VALUE, "Invalid scancode:%d",scancode);
        return NULL;
    }

    const int key = _glfw.h2co3_launcher.keycodes[scancode];
    return _glfw.h2co3_launcher.keynames[key];
}

int _glfwPlatformGetKeyScancode(int key)
{
    return _glfw.h2co3_launcher.scancodes[key];
}
                                          ...
                                          VkSurfaceKHR* surface)
{
    ...
    sci.window = window->h2co3_launcher.handle;
...
GLFWAPI struct ANativeWindow* glfwGetH2CO3Window(GLFWwindow* handle)
{
    ...
    return window->h2co3_launcher.handle;
}
