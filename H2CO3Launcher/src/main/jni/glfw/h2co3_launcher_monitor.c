...
static void modeChangeHandle(int width, int height, void* data)
{
    ...
    monitor->h2co3_launcher.currentMode = monitor->modeCount - 1;
}
...
void _glfwPlatformGetMonitorWorkarea(_GLFWmonitor* monitor, int* xpos, int* ypos, int* width, int* height)
{
    ...
    if (ypos)
        *ypos = 0;
    if (width)
        *width = monitor->modes[monitor->h2co3_launcher.currentMode].width;
    if (height)
        *height = monitor->modes[monitor->h2co3_launcher.currentMode].height;
}
...
void _glfwPlatformGetVideoMode(_GLFWmonitor* monitor, GLFWvidmode* mode)
{
    ...
    *mode = monitor->modes[monitor->h2co3_launcher.currentMode];
}

GLFWbool _glfwPlatformGetGammaRamp(_GLFWmonitor* monitor, GLFWgammaramp* ramp)
{
    _glfwInputError(GLFW_PLATFORM_ERROR,
                    "H2CO3: Gamma ramp access not supported");
    return GLFW_FALSE;
}
