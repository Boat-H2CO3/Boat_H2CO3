...
static void makeContextCurrentOSMesa(_GLFWwindow* window)
{
    if (window)
    {
        ...
        if (strcmp(getenv("LIBGL_STRING"), "VirGLRenderer") != 0) {
            ANativeWindow_lock(window->h2co3_launcher.handle, &buf, NULL);
...
static void swapBuffersOSMesa(_GLFWwindow* window)
{
    ...
    if (strcmp(getenv("LIBGL_STRING"), "VirGLRenderer") == 0) {
        ...
        window->context.Finish();
        ANativeWindow_unlockAndPost(window->h2co3_launcher.handle);
        ANativeWindow_lock(window->h2co3_launcher.handle, &buf, NULL);
    }
}
                                  ...
                                  const _GLFWfbconfig* fbconfig)
{
    ...
    if (strcmp(renderer, "VirGLRenderer") == 0) {
        ...
        ANativeWindow_setBuffersGeometry(window->h2co3_launcher.handle, 0, 0, vid);
