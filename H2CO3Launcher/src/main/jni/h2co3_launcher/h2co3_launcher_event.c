...
void h2co3SetCursorMode(int mode) {
    if (h2co3Launcher == NULL || h2co3Launcher->android_jvm == 0) {
        H2CO3_INTERNAL_LOG("h2co3SetCursorMode:h2co3Launcher or android_jvm is null");
        return;
    }
    JNIEnv *env = 0;
    jint result = (*h2co3Launcher->android_jvm)->AttachCurrentThread(h2co3Launcher->android_jvm, &env, 0);
    if (result != JNI_OK || env == 0) {
        H2CO3_INTERNAL_LOG("h2co3SetCursorMode:Failed to attach thread");
        abort();
    }
    ...
    if (class_H2CO3LauncherActivity == 0) {
        H2CO3_INTERNAL_LOG( "h2co3SetCursorMode:class_H2CO3LauncherActivity is null");
        abort();
    }
    ...
    (*env)->CallVoidMethod(env, h2co3Launcher->h2co3Activity, H2CO3LauncherActivity_setCursorMode, mode);
...
int h2co3PollEvent(H2CO3LauncherEvent *event) {
    ...
    if (pthread_mutex_lock(&h2co3Launcher->event_queue_mutex)) {
        H2CO3_INTERNAL_LOG( "h2co3PollEvent:Failed to acquire mutex");
        return 0;
    }
    char c;
    int ret = 0;
    ...
    if (pthread_mutex_unlock(&h2co3Launcher->event_queue_mutex)) {
        H2CO3_INTERNAL_LOG("h2co3PollEvent:Failed to release mutex");
        return 0;
    }
    return ret;
}
